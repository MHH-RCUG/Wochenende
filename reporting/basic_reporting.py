...
@click.option('--input_file',
              help='The output file of Wochenende. Either *.bam.txt or .bam (use with --slow flag only)')
    ...
    if slow:
        ...
        if sequencer == 'illumina':
            ...
            for seq_record in SeqIO.parse(refseq_file, 'fasta'):
                ...
                joined_reads = ''.join([read.query_sequence for read in
                                        pysam.AlignmentFile(input_file, 'rb').fetch(contig=seq_record.name)])
            ...
            res_df['RPMM'] = res_df['read_count'] / (res_df['chr_length'] / 1000000) * res_df[
                'read_count'].sum() / 1000000
            ...
            res_df_filtered_and_sorted.to_csv(f'{sample_name}.reporting.sorted.csv', sep='\t', float_format='%.1f',
                                              index=False)
        ...
        if sequencer == 'illumina':
            ...
            gc_ref_list = []
            for seq_record in SeqIO.parse(refseq_file, 'fasta'):
                gc_ref_list[seq_record.name] = SeqUtils.GC(str(seq_record.seq).replace('N', ''))
