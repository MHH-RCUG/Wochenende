...
@click.command()
@click.option('--slow', dafault=False, help='Use this falg if you want to process the bam file instead of *.bam.txt')
	...
	if slow:
		...
		if sequencer == 'illumina':
			...
			for seq_record in SeqIO.parse(refseq_file, 'fasta'):
				...
				readcount_list.append(pysam.AlignmentFile(input_file, 'rb').count(contig=seq_record.name))
		...
		if sequencer == 'illumina':
			# illumina reporting
			click.echo('starting illumina reporting')
			res_df = pd.read_csv(input_file, sep='\t', header=None, names=['species', 'size', 'readcount'])
			gc_ref_list = []
			for seq_record in SeqIO.parse(refseq_file, 'fasta'):
				gc_ref_list.append(SeqUtils.GC(seq_record.seq))
