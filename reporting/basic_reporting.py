...
def reporting(slow, input_file, refseq_file, sequencer, sample_name):
	if slow:
		...
		if sequencer == 'illumina':
			...
			res_df.to_csv(f'{sample_name}.basic_reporting.csv', sep='\t', index=False)
		...
		if sequencer == 'illumina':
			...
			res_df = pd.read_csv(input_file, sep='\t', header=None, names=['species', 'size', 'readcount'], usecols=[0,1,2])[:-1]
